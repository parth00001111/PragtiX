...
export default function AuthModal({ onClose, onAuthenticated }) {
  ...
  const submit = async (event) => {
    ...
    try {
      if (mode === 'login') {
        ...
        if (data.requiresEmailConfirmation) setConfirmation(data.message)
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}>
    <div className="auth-modal" role="dialog" aria-modal="true" aria-labelledby="auth-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      {confirmation ? <div className="success-state"><CheckCircle2 size={48} /><h2>Check your inbox</h2><p>{confirmation}</p><button className="primary" onClick={() => switchMode('login')}>Go to sign in</button></div> : <>
        ...
        <form onSubmit={submit}>
          ...
          {mode === 'signup' && <label>I am joining as<select name="role" defaultValue="CITIZEN"><option value="CITIZEN">Citizen / community member</option><option value="UNIVERSITY">University / HEI representative</option><option value="INDUSTRY">Industry / startup partner</option></select></label>}
